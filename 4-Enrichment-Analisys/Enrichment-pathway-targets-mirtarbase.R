...
pathways = pathways[-2]
# Load pathway info
pathway_info_mirtarbase <- read.table("./results/processed-pathways/pathways-info-targets-mirtarbase.csv", sep=";", quote="\"", stringsAsFactors=FALSE, header = TRUE)
...
for (i in 1:dim(targets_matrix)[1]) { #rows are mirna
  for (j in 1:dim(targets_matrix)[2]) { #cols are pathways
    ...
    ngenes_pathway = pathway_info_mirtarbase$Total[which (pathway_info_mirtarbase$Id == pathways$Id[j])]

    if (length(which (mirna_total_targets$mirnaid==target_data[i+targetoffset,1])) != 0) {
...
tiff("./results/mirnascore-mirtarbase.tif", width = 12, height = 6,  units="in", res = 300)

barplot(-log(significantmirnasscore$pvalue),
        names.arg = as.vector(row.names(significantmirnasscore)),
        space=1, border=1,
        main="",
        ylab="-log(p-value)",
        las=3,
        ...
        cex.names=0.7)
dev.off()
write.table(mirnasscore,"./results/mirnascore-mirtarbase.csv",sep = ";")
